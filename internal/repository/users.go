...
func (r *GophermartRepository) AddUser(login, passwordHash string) (uuid.UUID, error) {
	args := pgx.NamedArgs{
		"login":         login,
...
func (r *GophermartRepository) GetByLogin(login string) (uuid.UUID, string, error) {
	args := pgx.NamedArgs{
		"login": login,
	}
